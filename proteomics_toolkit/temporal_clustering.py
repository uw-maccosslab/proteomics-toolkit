...
class TemporalClusteringConfig:
    ...
    # Enrichment settings
    enrichr_libraries: List[str] = field(
...
def calculate_temporal_means(
    ...
    for subject in unique_subjects:
        ...
        subject_samples = [s for s in sample_cols if sample_subjects.get(s) == subject]
...
def determine_optimal_clusters(
    ...
    if plot:
        ...
        axes[1].axhline(
            y=best_sil - silhouette_tolerance,
            color="gray",
            linestyle=":",
        ...
        axes[1].axvline(x=optimal_k, color="red", linestyle="-", linewidth=2, label=f"Selected: k={optimal_k}")
        ...
        if optimal_k == best_sil_k:
            ...
            selection_reason = (
                f"Selected k={optimal_k} (highest k within silhouette tolerance of {silhouette_tolerance})"
            )

        plt.suptitle(f"Cluster Selection Analysis\n{selection_reason}", fontsize=14, fontweight="bold")
        plt.tight_layout()
...
def plot_silhouette_analysis(
    ...
    for i in range(n_clusters):
        ...
        ax1.text(-0.05, y_lower + 0.5 * cluster_size, cluster_label, fontsize=10, fontweight="bold", va="center")
    ...
    ax2.axvline(x=silhouette_avg, color="red", linestyle="--", linewidth=2, label=f"Overall avg: {silhouette_avg:.3f}")
    ...
    plt.suptitle(f"{title}\n{interpretation} (avg silhouette = {silhouette_avg:.3f})", fontsize=14, fontweight="bold")
    plt.tight_layout()
...
def cluster_temporal_trends(
    temporal_df: pd.DataFrame, week_columns: List[str], config: Optional[TemporalClusteringConfig] = None
    ...
    if config.auto_detect_clusters:
        ...
        n_clusters, cluster_selection_fig, _ = determine_optimal_clusters(
            X_scaled, k_range=(config.min_clusters, config.max_clusters), random_seed=config.random_seed, plot=True
        )
...
def name_clusters_by_pattern(X_scaled: np.ndarray, labels: np.ndarray, week_columns: List[str]) -> Dict[int, str]:
    """
...
def classify_trend_pattern(
    trajectory: np.ndarray, config: Optional[TemporalClusteringConfig] = None
) -> Tuple[str, float]:
    """
...
def merge_with_statistics(
    temporal_df: pd.DataFrame, stats_df: pd.DataFrame, cluster_labels: np.ndarray, cluster_names: Dict[int, str]
) -> pd.DataFrame:
    """
...
def filter_significant_proteins(
    merged_df: pd.DataFrame, config: Optional[TemporalClusteringConfig] = None
) -> pd.DataFrame:
    """
...
def query_enrichr(gene_list: List[str], config: Optional[TemporalClusteringConfig] = None) -> Dict[str, List]:
    """
    ...
    genes_str = "\n".join(clean_genes)
    payload = {"list": (None, genes_str), "description": (None, "Temporal Clustering Analysis")}
    ...
    for library in config.enrichr_libraries:
        try:
            ...
            response = requests.get(
                f"{ENRICHR_URL}/enrich", params={"userListId": user_list_id, "backgroundType": library}, timeout=30
            )
...
def parse_enrichr_results(results: Dict[str, List], config: Optional[TemporalClusteringConfig] = None) -> pd.DataFrame:
    """
...
def plot_enrichment_barplot(
    enrichment_df: pd.DataFrame, title: str, top_n: int = 15, figsize: Tuple[int, int] = (12, 8)
) -> Optional[Figure]:
    """
    ...
    legend_elements = [
        ...
        for lib, c in library_colors.items()
        if lib in plot_df["Library"].values
    ]
    if legend_elements:
...
def plot_enrichment_comparison(
    enrichment_dict: Dict[str, pd.DataFrame], title: str, top_n_per_group: int = 5, figsize: Tuple[int, int] = (14, 10)
) -> Optional[Figure]:
    """
    ...
    unique_terms = (
        combined_df.groupby("Term")["Combined_Score"].max().sort_values(ascending=False).head(20).index.tolist()
    )
...
def run_temporal_analysis(
    ...
    temporal_df, unique_weeks = calculate_temporal_means(
        data_df, metadata_dict, week_column, subject_column=config.subject_column
    )
    ...
    if config.auto_detect_clusters:
        print(
            f"\n2. Auto-detecting optimal number of clusters (range: {config.min_clusters}-{config.max_clusters})...",
            flush=True,
        )
    else:
    ...
    merged_df = merge_with_statistics(temporal_df, stats_df, cluster_labels, cluster_names)
    ...
    if run_enrichment and len(sig_df) > 0:
        ...
        for cluster_name, enrich_df in enrichment_results.items():
            if not enrich_df.empty:
                fig = plot_enrichment_barplot(enrich_df, f"{treatment_name}: {cluster_name} Enrichment")
        ...
        if any(not df.empty for df in enrichment_results.values()):
            fig_comparison = plot_enrichment_comparison(
                enrichment_results, f"{treatment_name}: Pathway Enrichment Comparison"
            )
