...
def query_enrichr(
    gene_list: List[str], config: Optional[EnrichmentConfig] = None, description: str = "Gene Set Enrichment Analysis"
) -> Dict[str, List]:
    """
    ...
    - Chen EY et al. (2013) Enrichr: interactive and collaborative HTML5 gene
      list enrichment analysis tool. BMC Bioinformatics.
    - Kuleshov MV et al. (2016) Enrichr: a comprehensive gene set enrichment
      analysis web server 2016 update. Nucleic Acids Research.
    """
    if config is None:
        config = EnrichmentConfig()
    ...
    # Submit gene list
    try:
        response = requests.post(f"{ENRICHR_URL}/addList", files=payload, timeout=config.timeout)
        if not response.ok:
...
def parse_enrichr_results(results: Dict[str, List], config: Optional[EnrichmentConfig] = None) -> pd.DataFrame:
    """
    ...
    for group_name in data_df[group_column].unique():
        ...
        if len(gene_list) >= config.min_genes:
            enrichment_df = run_enrichment_analysis(
                gene_list, config, description=f"Group: {group_name}", verbose=verbose
            )
...
def merge_enrichment_results(enrichment_dict: Dict[str, pd.DataFrame], add_group_column: bool = True) -> pd.DataFrame:
    """
