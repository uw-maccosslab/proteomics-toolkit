...
def validate_metadata_data_consistency(
    ...
    results = {"is_valid": True, "errors": [], "warnings": [], "diagnostics": {}}
    ...
    if missing_samples:
        error_msg = (
            f"Found {len(missing_samples)} samples in metadata that have no corresponding "
    ...
    if missing_control_samples:
        ...
        error_msg = (
            f"Found {len(missing_control_samples)} control samples in metadata that are missing "
            f"from protein data. Control types affected: {list(control_types)}. "
            f"Missing samples: {[s for s, _ in missing_control_samples[:3]]}"
            f"{'...' if len(missing_control_samples) > 3 else ''}"
        )
        results["errors"].append(error_msg)
        results["is_valid"] = False
...
def enhanced_sample_processing(
    ...
    # Clean sample names
    cleaned_sample_names_dict = toolkit_module.data_import.clean_sample_names(sample_columns, auto_detect=True)
    ...
    if expected_control_count != actual_control_count:
        warning_msg = (
            f"Expected {expected_control_count} control samples to be classified, "
    ...
    print(
        f"Done: Enhanced sample processing complete: "
        f"{len(final_sample_columns)} samples, {len(sample_metadata)} with metadata"
    )

    return final_sample_columns, sample_metadata, group_distribution, control_samples, study_samples, group_colors


def generate_sample_matching_diagnostic_report(validation_results: Dict, output_file: Optional[str] = None) -> str:
    """
